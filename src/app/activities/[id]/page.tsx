...
export default async function ActivityIntroPage({
  ...
  const [{ qr }, session, activity, myCert] = await Promise.all([searchParams, auth(), getActivity(id), getMyCertificate(id)]);
  ...
  return <ActivityIntro activity={activity} user={user} fromQR={qr === "1"} done={!!myCert} />;
}
